...
async def run_scout_endpoint(current_user: User = Depends(get_current_user)):
    asyncio.create_task(_run_pipeline(current_user.id))
    return {"status": "started"}


async def _run_pipeline(user_id: int):
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            voice_profile = (user.voice_profile or "") if user else ""
            ...
            drafts_data = await asyncio.to_thread(run_draft, post_models, voice_profile, 3)
